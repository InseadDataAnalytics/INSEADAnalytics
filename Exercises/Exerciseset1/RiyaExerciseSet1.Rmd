...
1. We use ncol(t(StockReturns)) to get the nuber of rows and columns of ths new matrix, as below - 
> ncol(t(StockReturns))
[1] 2774
> nrow(t(StockReturns))
[1] 1
...
1. In the data.R file, we have to change the start date here "startDate = "2005-01-01"" to the desired date of 2001-01-01. We make this change and run the .R file again to procure the data for this new date band.

2. website_used = "yahoo"
mytickers = c("SPY", "MSFT")
...
2. 
sum(myData != StockReturns[1:40,])
The sum is still 20 since there is nothing in the other 20 columns. 
...
myData <- read.csv(file = "MarriageAge.csv", header = TRUE, sep=";")

nrow(myData)
ncol(myData)
apply(myData,1,mean)
apply(myData, 1, class)
<br>
