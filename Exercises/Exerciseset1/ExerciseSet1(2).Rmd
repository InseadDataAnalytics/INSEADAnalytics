...
1. This is done in dataSet1.R. Apple's ticker, along with the S&P 500, is included into the vector "myticket". After creating the matrix with only SPY as a column, the code adds a column that corresponds to the Apple ticker. This is done at line 23.

2. The cumulative returns of Apple is `r round(100*sum(StockReturns[,2]),1)`%;The average daily return of Apple is `r round(100*mean(StockReturns[,2]),3)`%; The standard deviation of the daily returns of Apple is `r round(100*sd(StockReturns[,2]),3)`%.

3. The cumulative returns of Yahoo is `r round(100*sum(StockReturns[,3]),1)`%;The average daily return of Yahoo is `r round(100*mean(StockReturns[,3]),3)`%; The standard deviation of the daily returns of Yahoo is `r round(100*sd(StockReturns[,3]),3)`%.
...
##### Answers
1. Number of rows:`nrow(transposedData)` ; number of columns: `ncol(transpoedData)` #how come r doesn't recognize transposedData when trying to calculate
...
##### Answers
1. Change startDate to 2001-01-01
