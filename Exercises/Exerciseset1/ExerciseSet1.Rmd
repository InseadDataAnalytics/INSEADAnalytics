...
<hr>
*Several R finance package are interesting including:*<br>
...
<hr>
Cumulative returns:`r round(100*sum(AaplStockReturns[,1]),1)`%<br>
Average daily returns:`r round(100*mean(AaplStockReturns[,1]),3)`%<br>
Standard deviation of the daily returns of Apple:`r round(100*sd(AaplStockReturns[,1]),3)`%<br>
<hr>
...
<br>
<hr>
We can get the number of row of the matrix transposedData using `nrow(transposedData)<br>
The transposedData matrix has nrow(transposedData) rows<br><br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio = apply(AaplStockReturns,1,mean)
names(portfolio) <- rownames(AaplStockReturns)
pnl_plot(portfolio)
```
