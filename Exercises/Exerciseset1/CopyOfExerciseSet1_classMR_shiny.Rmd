...
For daily stock variations: https://github.com/frederickpelchat/quantitative-finance.git
For personal finance: https://github.com/shenie/romey.git
```
...
1. Coding in dataSet1.R includes command: mytickers = c("SPY", "AAPL"), AAPL being apple.
...
**Your Answers here:**
```{r eval = FALSE, echo=TRUE}
1. We can use the commands nrow(transposedData) and ncol(transposedData). Answers are 3 and 2774 respectively. 
```
2. For S&P and Apple
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio = apply(StockReturns,1,mean)
...
**Your Answers here:**
```{r eval = FALSE, echo=TRUE}
1. You need to change line 10: startDate = "2001-01-01". To get the new exercise set, you should run dataSet1.R and proceed with the same commands we have done until now but with the new data.
```
2. New tickers include MMM, FB, LULU. 
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio = apply(StockReturns,1,mean)
names(portfolio) <- rownames(StockReturns)
pnl_plot(portfolio)
...
**Your Answers here:**
```{r eval = FALSE, echo=TRUE}
1. It gives 20 as a result, corresponding to the sum of matches that correspond between myData and StockReturns.
2. It reads the following: Error in FUN(left, right) : 
