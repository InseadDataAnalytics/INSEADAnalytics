...
````THURSDAY`````
``[21:22, 1/26/2017] +65 9297 4153: ```i will be there
````21:22`````
``[21:22, 1/26/2017] Kate Tsunoda: ```Yaayyy
````21:22`````
``[21:48, 1/26/2017] +65 9297 4153: ```Thanks for the invite :)
````21:48`````
````FRIDAY`````
``[19:00, 1/27/2017] Kate Tsunoda: ```Hi Zhenfu! I'm sorry, but I won't be able to join for dinner this evening. Very sorry to have to miss it!
````19:00`````
``[19:08, 1/27/2017] +65 9297 4153: ```Come for some food at least ? 😀
````19:08`````
````SUNDAY`````
``[01:02, 1/29/2017] +65 9297 4153: ```I will reach ard 730!
````01:02`````
``[01:08, 1/29/2017] Kate Tsunoda: ```Can you please bring baguettes ?
````01:08`````
``[01:08, 1/29/2017] +65 9297 4153: ```How many
````01:08`````
``[01:10, 1/29/2017] Kate Tsunoda: ```2-3?
````01:10`````
``[01:10, 1/29/2017] Kate Tsunoda: ```Maybe some cheese ?
````01:10`````
``[01:11, 1/29/2017] +65 9297 4153: ```Ok
````01:11`````
``[01:11, 1/29/2017] +65 9297 4153: ```Whst cheese
````01:11`````
``[01:14, 1/29/2017] Kate Tsunoda: ```Thank you!
````01:14`````
``[01:14, 1/29/2017] Kate Tsunoda: ```Wouldn't have asked, but I am bicycling to Fonty and won't have time before Leo arrives
````01:14`````
``[06:29, 1/29/2017] +65 9297 4153: ```Where is the placr again
````06:29`````
 ````116 kB`````

````06:31`````
``[23:36, 1/29/2017] Kate Tsunoda: ```Hi Zhenfu, hope you're Sunday is good! :) is there further work needed on the big data group project?
````23:36`````
``[23:39, 1/29/2017] +65 9297 4153: ```Nope i dun think so
````23:39`````
``[23:39, 1/29/2017] +65 9297 4153: ```Its pretty much it
````23:39`````
``[23:39, 1/29/2017] Kate Tsunoda: ```Ok cool! :)
````23:39`````
````YESTERDAY`````
``[00:02, 1/30/2017] +65 9297 4153: ```How ru feeling
````00:02`````
``[00:13, 1/30/2017] Kate Tsunoda: ```Woke up with a very sore, inflamed throat
````00:13`````
``[00:13, 1/30/2017] Kate Tsunoda: ```otherwise, great 🙂
````00:13`````
``[00:15, 1/30/2017] +65 9297 4153: ```Oh man
````00:15`````
``[06:42, 1/30/2017] +65 9297 4153: ```Hey kate my classes end at 7 and there is a microsoft talk till 815. R u alright to meet after ?
````06:42`````
``[06:43, 1/30/2017] Kate Tsunoda: ```Yes! Let me move it.
````06:43`````
``[06:43, 1/30/2017] +65 9297 4153: ```Ok cool
````06:43`````
``[06:45, 1/30/2017] Kate Tsunoda: ```Thank you!!
````06:45`````
``[03:18, 1/31/2017] Kate Tsunoda: ```Hey! How long do you want to stay in the talk?
````03:18`````
``[03:23, 1/31/2017] +65 9297 4153: ```Lets go if u want
````03:23`````
``[03:23, 1/31/2017] Kate Tsunoda: ```Ok! Let's head out
````03:23`````
``[03:51, 1/31/2017] Kate Tsunoda: ```Component 1: Bonds
...
````03:51`````
``[03:56, 1/31/2017] Kate Tsunoda: ```5.The components have changed both in composition as well as assigned weights. Therefore, the results imply that assets are correlated differently during bear years as compared to bull years.
````03:56`````
````1 UNREAD MESSAGE`````
``[04:07, 1/31/2017] Kate Tsunoda: ```<br>
Component 1: Bonds
<br>
Component 2: Currencies
<br>
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data = data.frame(Variance_Explained_Table_bull)
m_bull<-gvisTable(show_data,options=list(width=1920, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m_bull,'chart')
```
<br>
...
```{r FigBull, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
...
Line_bull         <- gvisLineChart(as.data.frame(df_bull), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Bull Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data = data.frame(Variance_Explained_Table_bear)
m_bear<-gvisTable(show_data,options=list(width=1920, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m_bear,'chart')
```
<br>
...
```{r FigCrisis, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
df_bear           <- cbind(as.data.frame(eigenvalues_bear), c(1:length(eigenvalues_bear)), rep(1, length(eigenvalues_bear)))
colnames(df_bear) <- c("eigenvalues", "components", "abline")
Line_bear         <- gvisLineChart(as.data.frame(df_bear), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Bear Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line_bear, 'chart')
```
<hr>
