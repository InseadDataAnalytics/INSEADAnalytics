...
Let's just re-use the code above. See also this [example of how to efficiently generate long reports by automatically re-using what you wrote before](http://inseaddataanalytics.github.io/INSEADAnalytics/reusability_example.html) (and [source code](https://github.com/InseadDataAnalytics/INSEADAnalytics/blob/master/CourseSessions/Sessions45/reusability_example.Rmd)). 
...
Here is the scree plot for the **pre-crisis years**:
<br> 
...
Here are the principal components for the **pre-crisis years**:
<br> 
...
And now we do the financial crisis years: note that **we use the exact same code, except we changed the definition of the variable `futures_data` in the very first line of the very first code chunk.** 
<br>

```{r echo=TRUE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
futures_data = futures_data_orig[as.Date(rownames(futures_data_orig)) > "2007-10-01" & as.Date(rownames(futures_data_orig)) < "2009-03-01", ]
...
Here is the scree plot for the **crisis years**:
<br> 
...
Here are the principal components for the **crisis years**:
<br> 
...
<br>
**Finally, let's get back our variables**


```{r echo=TRUE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
futures_data = futures_data_orig
Variance_Explained_Table_results = Variance_Explained_Table_results_orig
Variance_Explained_Table = Variance_Explained_Table_orig
eigenvalues = eigenvalues_orig
corused = corused_orig
Rotated_Results = Rotated_Results_orig
Rotated_Factors = Rotated_Factors_orig
```


*Question 5:*

So many things to say here! For example: does the largest eigenvalue increase during the crisis years? If so, this means that **more of the risk (variance) across all assets is explained by one (the first) risk factor** - does this make sense? What is the main risk factor (e.g. first principal component) during the specific crisis? How about in "normal times"? Is it commodities (e.g. oil), fixed income, or equities? How are the relative weights in the principal components changing across different periods? Is long-term or short-term bonds that "matter" more? Are the crisis of 2000, 2007, and possibly today, driven by the same factors? 

*Question 6:*

An interative tool and source code for this exercise are already posted on the course website.
...
Try for example (in the Console, line by line) the code above with `MAfreq<-seq(10,250,by=5)`
