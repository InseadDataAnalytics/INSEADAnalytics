...
**Your Answers here:**
<br> Q1. 6 components
<br> Q2. 5 yr and 10 yr T notes
<br> Q3. Yes, these are the Principal Components Factors of our analysis
<br> Q4a.
**Before Crisis**
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE, results='asis'}
...
pre_crisis_show_data = data.frame(cbind(colnames(pre_crisis_data), round(cor(pre_crisis_data),2)))
pre_crisis_m<-gvisTable(pre_crisis_show_data,options=list(width=1920, height=min(400,27*(nrow(pre_crisis_show_data)+1)),allowHTML=TRUE))
print(pre_crisis_m,'chart')
...
pre_crisis_m<-gvisTable(pre_crisis_show_data,options=list(width=1920, height=min(400,27*(nrow(pre_crisis_show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(pre_crisis_m,'chart')
...
pre_crisis_Line         <- gvisLineChart(as.data.frame(pre_crisis_df), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
...
colnames(Pre_Crisis_Rotated_Factors)<-paste("Component",1:ncol(Pre_Crisis_Rotated_Factors),sep=" ")

pre_crisis_sorted_rows <- sort(Pre_Crisis_Rotated_Factors[,1], decreasing = TRUE, index.return = TRUE)$ix
Pre_Crisis_Rotated_Factors <- Pre_Crisis_Rotated_Factors[pre_crisis_sorted_rows,]
Pre_Crisis_Rotated_Factors[abs(Pre_Crisis_Rotated_Factors) < 0.3]<-NA

pre_crisis_show_data <- Pre_Crisis_Rotated_Factors 
pre_crisis_show_data <-cbind(rownames(pre_crisis_show_data),pre_crisis_show_data)
colnames(pre_crisis_show_data)<-c("Variables",colnames(Pre_Crisis_Rotated_Factors))
pre_crisis_m<-gvisTable(pre_crisis_show_data,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'))
print(pre_crisis_m,'chart')
```
...
crisis_data_show_data = data.frame(cbind(colnames(crisis_data), round(cor(crisis_data),2)))
crisis_data_m<-gvisTable(crisis_data_show_data,options=list(width=1920, height=min(400,27*(nrow(crisis_data_show_data)+1)),allowHTML=TRUE))
print(crisis_data_m,'chart')

Crisis_Data_Variance_Explained_Table_results <-PCA(crisis_data, graph=FALSE)
Crisis_Data_Variance_Explained_Table <-cbind(paste("component",1:ncol(crisis_data),sep=" "),Crisis_Data_Variance_Explained_Table_results$eig)
Crisis_Data_Variance_Explained_Table<-as.data.frame(Crisis_Data_Variance_Explained_Table)
colnames(Crisis_Data_Variance_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")

crisis_data_show_data = data.frame(Crisis_Data_Variance_Explained_Table)
crisis_data_m<-gvisTable(crisis_data_show_data,options=list(width=1920, height=min(400,27*(nrow(crisis_data_show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(crisis_data_m,'chart')

crisis_data_eigenvalues  <- Crisis_Data_Variance_Explained_Table[,2]

crisis_data_df           <- cbind(as.data.frame(crisis_data_eigenvalues), c(1:length(crisis_data_eigenvalues)), rep(1, length(crisis_data_eigenvalues)))
colnames(crisis_data_df) <- c("eigenvalues", "components", "abline")
Crisis_Data_Line         <- gvisLineChart(as.data.frame(crisis_data_df), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Crisis_Data_Line, 'chart')

crisis_data_corused = cor(crisis_data[,apply(crisis_data!=0,2,sum) > 10, drop=F])
Crisis_Data_Rotated_Results <- principal(crisis_data_corused, nfactors=20, rotate="varimax",score=TRUE)
Crisis_Data_Rotated_Factors <- round(Crisis_Data_Rotated_Results$loadings,2)
Crisis_Data_Rotated_Factors <-as.data.frame(unclass(Crisis_Data_Rotated_Factors))
colnames(Crisis_Data_Rotated_Factors)<-paste("Component",1:ncol(Crisis_Data_Rotated_Factors),sep=" ")

crisis_data_sorted_rows <- sort(Crisis_Data_Rotated_Factors[,1], decreasing = TRUE, index.return = TRUE)$ix
Crisis_Data_Rotated_Factors <- Crisis_Data_Rotated_Factors[crisis_data_sorted_rows,]
Crisis_Data_Rotated_Factors[abs(Crisis_Data_Rotated_Factors) < 0.3]<-NA

crisis_data_show_data <- Crisis_Data_Rotated_Factors 
crisis_data_show_data <-cbind(rownames(crisis_data_show_data),crisis_data_show_data)
colnames(crisis_data_show_data)<-c("Variables",colnames(Crisis_Data_Rotated_Factors))
crisis_data_m<-gvisTable(crisis_data_show_data,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'))
print(crisis_data_m,'chart')

```


Smaller number of set of components explain the market during crisis when compared to the component mix before crisis
