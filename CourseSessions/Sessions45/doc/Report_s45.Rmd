...
In our case, assuming we decided we use the `r numb_clusters_used` segments found using hierarchical clustering as outlined above (similar profiling can be done with the results of kmeans or other segmentation methods), we can see how the responses to our survey differ across segments. The summary statistics of our data for the total population as well as within each customer segment are:
