...
tags$style(type="text/css", ".formattable_widget { overflow:auto; max-height:425px; }")
...
renderFormattable(dformat({
  num <-  nrow(ProjectDataX()) - 1
  df1 <- cbind(as.data.frame(Hierarchical_ClusterX()$height[length(Hierarchical_ClusterX()$height):1]), c(1:num))
...
We now select the original raw attributes to use for the profiling of the segments (the "profiling attributes")
...
observeEvent(ProjectDataX(), {
  attributes <- colnames(ProjectDataX())
  updateSelectizeInput(session, "profile_attributes_used", choices=attributes, selected=attributes)
})
