...
```{r}
cluster_memberships_hclust <- as.vector(cutree(Hierarchical_Cluster, k=numb_clusters_used)) # cut tree into numb_clusters_used clusters
...
ggplot(melt(as.data.frame(cbind(id=1:nrow(Cluster_Profile_standar_mean), Cluster_Profile_standar_mean)), id="id"), aes(x=id, y=value, colour=variable)) + geom_line() + scale_colour_manual(values = c("cyan", "blue","red")) + xlab("Profiling variables (standardized)") + ylab("Mean of cluster")
```
