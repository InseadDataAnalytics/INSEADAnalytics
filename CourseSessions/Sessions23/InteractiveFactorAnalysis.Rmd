...
tags$style(type="text/css", ".formattable_widget { overflow:auto; max-height:415px; }")
...
observeEvent(ProjectDataX(), {
  ...
  updateSelectizeInput(session, "factorAttributes", choices=attributes, selected=head(attributes, MAX_COLUMNS))
})
...
Here is how the first `r MAX_ROWS` rows look like:
...
For our data, using the rotated factors we selected, we can create a new dataset where our observations are as follows (for the first `r MAX_ROWS` observations):
