...
ProjectData <- read.csv(paste0("data/", datafile_name, ".csv"), sep=";", dec=",") # this contains only the matrix ProjectData
...
```{r}
rownames(ProjectDataFactor) <- paste0("data ", sprintf("%02i", 1:nrow(ProjectDataFactor)))
...
```{r figure02}
if(ncol(NEW_ProjectData) == 1) {
  NEW_ProjectData <- cbind(
    NEW_ProjectData, 
    list(`Derived Variable (Factor) 2`=NEW_ProjectData$`Derived Variable (Factor) 1`)
  )
}
  
ggplot(
  as.data.frame(NEW_ProjectData), 
  aes(
    x = `Derived Variable (Factor) 1`,
    y = `Derived Variable (Factor) 2`
  )) +
geom_point()
```
