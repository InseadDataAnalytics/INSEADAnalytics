...
if (0){
  #tmpx = t(apply(ProjectData[,7:12], 1, function(r) matrix(c(sum(r > 0), sum(r==0), sum(r==-2), sum(r==-1)), nrow=1)))
...
boxplots_independent_variables = c(1:7) # use only the PAY_ variables
...
if (reg_logreg){
  ...
  cv.out <- cv.glmnet(est_data[,independent_variables],estimation_data[,dependent_variable],alpha=1,family="binomial",type.measure = "mse",intercept=FALSE) 
