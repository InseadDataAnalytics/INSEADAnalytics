...
#datafile_name = "/Users/spyroszoumpoulis1/Documents/INSEAD Teaching/Data Science for Business/INSEADAnalytics/Data/UCI_Credit_Card.csv"
...
actual_1_predict_0 = -1000
actual_0_predict_1 = 0
actual_0_predict_0 = 200
...
We typically refer to the three data samples as **estimation_data** (`r estimation_data_percent`% of the data in our case), **validation_data**  (`r validation_data_percent`% of the data) and **test_data** (the remaining `r 100 - estimation_data_percent  -  validation_data_percent`% of the data).
...
**Logistic Regression**: For example, this is the logistic regression parameters for our data:
```{r echo=FALSE}
...
The estimated probability a validation observation belongs to class 1 (e.g., the estimated probability that the customer defaults) for the first few validation observations, using the logistic regression above, is:
...
**CART**: Running a basic CART model with complexity control cp=`r CART_cp`,  leads to the following tree (**NOTE**: for better readability of the tree figures below,  we will rename the independent variables as IV1 to `r paste("IV", length(independent_variables), sep="")` when using CART):
...
One can also use the percentage of data in each leaf of the tree to get an estimate of the probability that an observation (i.e., customer) belongs to a given class.  The **purity of the leaf** can indicate the probability an observation which "reaches that leaf" belongs to a class. In our case, the probability our validation data belong to class 1 (i.e., the customer is likely to default) for the first few validation observations, using the first CART above, is:
...
Let's see in our case how the **Hit Ratio, Confusion Matrix, ROC Curve, Lift Curve, and Profit Curve** look like for our test data. 
...
1. Is the performance in the test data similar to the performance in the validation data above? 
2. Should we expect the performance of our classification model to be close to that in our test data when we deploy the model in practice? Why or why not? What should we do if they are different?
