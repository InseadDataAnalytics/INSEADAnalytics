...
As noted in the analysis above, each of the derived new variables is a linear combination of the original ones. In practice one would like to have derived variables that use only a few of the original raw attributes - while each new derived variable using different subsets of the original raw attributes. 
...
For example for our data one can "rotate" (mathematically) the original factors to generate new ones wich are easier to interpret while capturing the same information from the raw data as the initial factors found.  For our data, the `r factors_selected` selected factors look as follows after the `r rotation_used` rotation: 
...
if(ncol(NEW_ProjectData) >= 2) {
  ...
  sca1 <- gvisScatterChart(df1, options=list(legend="none",
                                             lineWidth=0, pointSize=12, hAxis.title="Derived Variable (Factor) 2",
