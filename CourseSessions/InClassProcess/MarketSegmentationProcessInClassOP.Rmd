...
rmarkdown::render("CourseSessions/InClassProcess/MarketSegmentationProcessInClassOP.Rmd") 
...
1.Questions 13 and 14, 16 and 17, and 27 and 28 have correlations over 0.6.
   Normally we use a threshold of 0.7 to signify autocorrelation risk.
   The correlated questions make intuitive sense:
          13 and 14 both relate to spending a large amount of money
          16 and 17 relate to knowledge about boating
          27 and 28 relate to boating as a passion.

2.These correlations imply that the questions are not independent and they may try to capture the same feature.
...
1. This table indicates how the percentage of variance explained changes as the number of components used increases. If there was a business requirement to explain a set percentage of the variance, we could use the table to know how many components to use.
2. We can use this plot to find the "elbow", which is where the optimal number of components is. After this point there is a diminishing return from adding more components. The plot will always be decreasing because the algorithm orders the components in descending order of usefulness (% of explained variance).
3. We would prefer to see an "L" shaped curve with a high percentage of variance explained by a few components. This is because it is easier to recommend business actions if there are fewer important components.
...
1. The first factor is mostly consisting of questions which relate to boats as status symbols - being big, fast, and to impress others. This makes business sense. The second factor expresses an interest in outdoor life as a hobby for free time. The third factor also makes business sense, and relates to the boat's brand and quality. Additionally, the fourth factor makes sense as the questions relate to being technically knowledgeable about boats. Component 5 is also useful as it reflects a focus on simple/budget functionality.
2. We should choose 5 factors. We tried a number of different combinations with different inputs (e.g. eigenvalue/variance, varimax/oblimin etc, and we repeatedly found similar results. Additionally the first 5 factors repeatedly express similar business concepts. From a business perspective, 5 is a reasonable number that is not too granular. Adding a 6th factor does not add value because it generally gave us just a single question.  
3. See question 1.
4. It is important to try different models to see what fits the data best. We need to balance explanatory power and complexity in our model. We are also able to use data science to identify the underlying motivations of people!
...
1. Person "Observation 1" loves his boat's quality; he is very knowledgeable and is proud of his status boat. He does not care about budget!
   Person "Observation 10" is not looking for a branded boat, but wants a simple boat for a fair price.
2. Factor 1 = Q21, Factor 2 = Q16, Factor 3 = Q4, Factor 4 = Q9, Factor 5 = Q2.
...
1. The numbers represent the weighted contribution of the question to that segment. Therefore, the higher the absolute value of the number, the more informative it is.
2. Segment 2 is very similar to the population mean across all features. Segment 4 scores highly on all variables from 13 to 17, typically an expert buyer who knows boats quite well.
...
1. We believe there are 3 distinct segments in the market. We selected this number because 
a) From looking at the dendrogram heights reasonable numbers would be between 3 and 5 segments, without increasing the height significantly more.
b) from interpreting the snake plot when running with 5 segments we see that segments 2,3,4 are more similar to each other, than to segments 1 and 5, this could merit selecting only 3 segments . 
c) Overall, 3 is a small enough number to have distinct business usage but not so small that it is not offering any granularity.
It will not always be the case that statistical/interpretation/business arguments all point to an identical number, as is the case here. On balance we choose 3 segments.

2. 
Segment 1: Scores above average in most areas, meaning that he wants a powerful status boat and is willing to spend more to get it.
Segment 2: Fairly representative of the population of boaters overall. If we split these into sub-segments (i.e. running with 5 segments) then sub-segment 2a has a specific aversion to repairs, segment 2b is happier to perform repairs, and segment 2c has a particular concern for a no-frills boat at a reasonable price.
Segment 3: Scores below average, he wants a functional boat and is not concerned with aesthetic value.

3. We tried with 10, 7, 5 and 3 factors. There is a trade-off between the specificity of the segment with its size. The more specific a segment is, the more you may be able to tailor your offering to their needs, but the downside would be a smaller segment. With fewer segments you risk aggregating customers to the point that not all of their desires are addressed accurately.

4. We recommend that the company launches a boat targetting the premium status-driven customers. This is because we find this to be the largest (1019 members) and least price-sensitive segment, where likely the highest margins are. These customers place value on the brand of their boat so it may not be advisable to also simultaneously introduce a cheaper boat, as this could undermine the premium branding.
...
1. You would select these values based on how you want to reward the algorithm to optimise for correct positives or correct negatives (ie minimising false positives or false negatives). The profit matrix relates very closely to business decisions because it tells us which customers we expect to buy our boat vs which customers actually buy our boat. For example, if we predict that a customer will buy our boat and they do not, this was wasted marketing effort and we want to penalise our algorithm to correct for this. 
2. If we increase the probability threshold we will have a lower absolute number of positives (we think only a few people will buy from us), but these positives will be more correct predictions. If we decrease the probability threshold we will get more false positives (we think more people will buy from us than actually do). This has implications for how we target our marketing spend - the very certain customers do not necessarily need to have marketing spend allocated to them, nor do the very unlikely customers.
...
1. The main purchase drivers are questions: 16 (brand equity), 2 (best in class customer service), 21 (offers boats that are easy to maintain), 17 (boat can handle rough weather), 22 (boat is easy to use), and 24 (low price). 
2. 
*
*
*
