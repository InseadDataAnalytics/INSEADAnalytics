...
1. *Part 1*: We use some of the survery questions (e.g. in this case the first 29 "attitude" questions) to construct key variables ("factors") using *dimensionality reduction* that will be used for the segmentation.
2. *Part 2*: We use the selected variables to segment the market using *cluster analysis* and profile the segments using profiling variables that we select. 
...
Before starting, make sure you have pulled the [course files](https://github.com/InseadDataAnalytics/INSEADAnalytics) on your github repository. In case you would like to manually go through the commands below, make sure you are in the directory of this file. For example, assuming we are now in the "MYDIRECTORY/INSEADAnalytics" directory, we can do these: 

```{r echo=TRUE, eval=FALSE, tidy=TRUE}
# FOR MANUALLY RUNNING THE CODE IN THIS FILE STEP-BY-STEP
#rm(list=ls()) # Clean up the memory, if we want to rerun from scratch
#getwd()
#setwd("CourseSessions/InClassProcess")
```
...
```{r setupdata, echo=FALSE, message=FALSE}
# Load the data
...
The code used here is along the lines of the code in the session 3-4 reading  [FactorAnalysisReading.Rmd](https://github.com/InseadDataAnalytics/INSEADAnalytics/blob/master/CourseSessions/Sessions23/FactorAnalysisReading.Rmd). All user inputs for this part should be selected here: 

```{r setupfactor, echo=FALSE, message=FALSE}
# SET UP OF ALL THE INPUTS FOR THIS PART

# Please ENTER then original raw attributes to use in this part
...
The code used here is along the lines of the code in the session 5-6 reading  [ClusterAnalysisReading.Rmd](https://github.com/InseadDataAnalytics/INSEADAnalytics/blob/master/CourseSessions/Sessions45/ClusterAnalysisReading.Rmd). All user inputs for this part should be selected here: 
...
We follow the process described in the [Clsuter Analysis ](https://github.com/InseadDataAnalytics/INSEADAnalytics/Sessions45/ClusterAnalysisReading.Rmd) reading. 
...
We need to define a distance metric that measures how different people (observations in general) are from each other. This can be an important choice. 
...
Here are the differences between the observations using the selected distance metric:
...
We can see the histogram of, say, the first 2 variables (can you change the code below to see other variables?)
...
We now need to select the clustering method to use, as well as the number of cluster: 
...
# Please ENTER the hierarchical clustering method to use in case we use hclust (options are:
# "ward", "single", "complete", "average", "mcquitty", "median" or "centroid")
# DEFAULT is "ward"
hclust_method = "ward"

# Please ENTER the kmeans clustering method to use in case we use kmeans (options are:
...
It may be useful to see the dendrogram from Hierarchical Clustering, to have a quick idea of how the data may be segmented and how many segments there may be. Here is the dendrogram for our data:
...
or, finally, the ratio of the average for each cluster to the average of the population (e.g. `avg(cluster)` `/` `avg(population)`) and explore a matrix as the following one:
...
We will now use the [classification methods ](http://inseaddataanalytics.github.io/INSEADAnalytics/Sessions67/ClassificationAnalysisReading.html) to understand the key purchase drivers for boats (a similar analysis can be done for recommendation drivers). For simplicity we do not follow the "generic" steps of classification discussed in that reading, and only consider the classification and purchase drivers analysis for the segments we found above. 


We are interested in understanding the purchase drivers, hence our **dependent** variable is column 82 of the Boats data (`r colnames(ProjectData)[82]`) - why is that? We will use only the subquestions of **Question 16** of the case for now, and also select some of the parameters for this analysis: 
...
This is the classification results without considering any customer segments:
...
And now the segment specific analysis and solution:
...
knitr::kable(round(tail(Log_Drivers,-1), 2))
