...
Running a basic CART model with complexity control cp=`r CART_cp`,  leads to the following tree (**NOTE**: for simplicity we will rename the independent variables as IV1 to `r paste("IV", length(independent_variables), sep="")` from now on):
...
estimation_Probability_class1_tree<-predict(CART_tree, data.frame(estimation_data_nolabel))[,2]
estimation_Probability_class1_tree_large<-predict(CART_tree_large, data.frame(estimation_data_nolabel))[,2]

validation_Probability_class1_tree<-predict(CART_tree, data.frame(validation_data_nolabel))[,2]
validation_Probability_class1_tree_large<-predict(CART_tree_large, data.frame(validation_data_nolabel))[,2]

test_Probability_class1_tree<-predict(CART_tree, data.frame(test_data_nolabel))[,2]
test_Probability_class1_tree_large<-predict(CART_tree_large, data.frame(test_data_nolabel))[,2]


estimation_prediction_class_tree=as.vector(estimation_Probability_class1_tree > Probability_Threshold)
estimation_prediction_class_tree_large=as.vector(estimation_Probability_class1_tree_large > Probability_Threshold)

validation_prediction_class_tree=as.vector(validation_Probability_class1_tree > Probability_Threshold)
validation_prediction_class_tree_large=as.vector(validation_Probability_class1_tree_large > Probability_Threshold)

test_prediction_class_tree=as.vector(test_Probability_class1_tree > Probability_Threshold)
test_prediction_class_tree_large=as.vector(test_Probability_class1_tree_large > Probability_Threshold)
...
```{r echo=FALSE, message=FALSE, prompt=FALSE, results='asis'}
logreg_solution <- glm(formula, family=binomial(link="logit"), data=estimation_data_nolabel)
...
estimation_Probability_class1_log<-predict(logreg_solution, type="response", newdata=estimation_data_nolabel[,independent_variables_nolabel])
validation_Probability_class1_log<-predict(logreg_solution, type="response", newdata=validation_data_nolabel[,independent_variables_nolabel])
test_Probability_class1_log<-predict(logreg_solution, type="response", newdata=test_data_nolabel[,independent_variables_nolabel])

estimation_prediction_class_log=as.vector(estimation_Probability_class1_log > Probability_Threshold)
validation_prediction_class_log=as.vector(validation_Probability_class1_log > Probability_Threshold)
test_prediction_class_log=as.vector(test_Probability_class1_log > Probability_Threshold)
...
validation_hit_rates = rbind(
  100*sum(validation_prediction_class1_tree==validation_actual)/length(validation_actual), 
  100*sum(validation_prediction_class1_tree_large==validation_actual)/length(validation_actual), 
...
estimation_hit_rates = rbind(
  100*sum(estimation_prediction_class1_tree==estimation_actual)/length(estimation_actual), 
  100*sum(estimation_prediction_class1_tree_large==estimation_actual)/length(estimation_actual), 
...
validation_prediction_class_tree<-predict(CART_tree, type="response", newdata=validation_data_nolabel[,independent_variables_nolabel])
validation_prediction_class_tree_large<-predict(CART_tree_large, type="response", newdata=validation_data_nolabel[,independent_variables_nolabel])
validation_prediction_class_log<-predict(logreg_solution, type="response", newdata=validation_data_nolabel[,independent_variables_nolabel])

validation_actual_class <- as.numeric(validation_data[,dependent_variable])

pred_tree <- prediction(validation_prediction_class_tree, validation_actual_class)
pred_tree_large <- prediction(validation_prediction_class_tree_large, validation_actual_class)
pred_log <- prediction(validation_prediction_class_log, validation_actual_class)
plot(performance(pred_tree, "tpr", "fpr"),  lty=1, add=FALSE, main="Lift Curve")
lines(performance(pred_tree_large, "tpr", "fpr"), col="red", lty=1, add=FALSE, main="Lift Curve")
lines(performance(pred_log, "tpr", "fpr"), col="blue", lty=1, add=FALSE, main="Lift Curve")
grid()
```
...
per_tree <- performance(pred_tree, "lift", "rpp")
per_tree_large <- performance(pred_tree_large, "lift", "rpp")
per_log <- performance(pred_log, "lift", "rpp")
per_tree@x.values[[1]] <- per_tree@x.values[[1]]*100
per_tree_large@x.values[[1]] <- per_tree_large@x.values[[1]]*100
per_log@x.values[[1]] <- per_log@x.values[[1]]*100
plot(per_tree,  lty=1, xlab="Caseload (%)", add=FALSE, main="Lift Curve")
legend("topright", c("glm"), col=rainbow(1, 1, .8), lty=1:1, title="Models", inset=c(0.05, 0.05))
grid()
lines(per_tree_large, col="red", lty=1, xlab="Caseload (%)", add=FALSE, main="Lift Curve")
lines(per_tree_log, col = "blue", lty=1, xlab="Caseload (%)", add=FALSE, main="Lift Curve")
```
