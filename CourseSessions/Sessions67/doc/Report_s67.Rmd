...
First, make sure the dependent variable is set up as a categorical 0-1 variable. In this illustrative example, the "intent to purchase" and "intent to recommend" are 0-1 variables: we will use the later as our dependent variable but a similar analysis could be done for the former. 
...
###for test data
class_percentages=matrix(c(sum(test_data[,dependent_variable]==1),sum(test_data[,dependent_variable]==0)), nrow=1); colnames(class_percentages)<-c("Class 1", "Class 0")
rownames(class_percentages)<-"# of Observations"
print(xtable(class_percentages ,caption=paste("Number of Observations per class in the Estimation Sample:",data_name,sep=" "), digits=1,),type="html",html.table.attributes = "class='table table-striped table-hover table-bordered'",caption.placement="top",comment = FALSE,include.rownames = TRUE)
```
...
test_hit_rates = rbind(
  ...
  100*sum(validation_prediction_class_log==test_actual)/length(test_actual)
  )
colnames(validation_hit_rates) <- "Hit Ratio"
rownames(validation_hit_rates) <- c("First CART", "Second CART", "Logistic Regression")

print(xtable(test_hit_rates ,caption=paste("Validation Data Hit Ratios for different classifiers tested:",data_name,sep=" "), digits=1,),type="html",html.table.attributes = "class='table table-striped table-hover table-bordered'",caption.placement="top",comment = FALSE,include.rownames = TRUE)
...
A simple benchmark to compare the performance of a classification model against is the **Maximum Chance Criterion**. This measures the proportion of the class with the largest size. For our validation data the largest group is people who do not intent do purchase a boat: `r sum(!validation_actual)` out of `r length(validation_actual)` people). Clearly without doing any discriminant analysis, if we classified all individuals into the largest group,  we could get a hit-rate of `r sum(!validation_actual)/length(validation_actual)`% - without doing any more work. Clearly, one should always have a hit rate of at least as much as the the Maximum Chance Criterion rate. 
...
Remember that each observation is classified in a group by our model according to the probabilities Pr(0) and Pr(1) and a chosen probability threshold. Typically we set the probability threshold to 0.5 - so that observations for which Pr(1) > 0.5 are classified as 1's. However, we can vary this threshold, for example if we are interested in correctly predicting all 1's but do not mind missing some 0's (and vice-versa) - can you think of such a scenario? When we change this threshold we get different values of hit rate, false positive and false negative rates, or any other performance metric. We can plot for example how the false positive versus true posititive rates change as we alter the probability threshold,  and generate the so called ROC curve. 
...
#perf <- performance(pred_log,"lift","tpr")
...
#y<-as.data.frame(perf@alpha.values)
#plot(x[,1],y[,1], main="lift curve for validation data", colorize=F)
...
Line    <- gvisLineChart(as.data.frame(df), xvar=c("False Positive rate CART"), yvar="True Positive CART", options=list(title='ROC Curve for CART', legend="right", width=600, height=400, hAxis="{title:'False Positive rate CART', titleTextStyle:{color:'black'}}", vAxes="[{title:'True Positive CART'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
############################
...
By changing the probability threshold, we can also generate the so called lift curve, which is useful for certain applications. For example, consider the case of capturing fraud by examining only a few transactions instead of every single one of them. In this case we may want to examine as few transactions as possible and capture the maximum number of frauds possible. We can measure the percentage of all frauds we capture if we only examine, say, x% of cases (the top x% in terms of Probability(fraud)). If we plot these points [percentage captured vs total examined] while we change the threshold, we get a curve that is called the **lift curve**. 
...
#lift_tree <- performance(pred_tree, "rpp", "tpr")
#lift_tree_large <- performance(pred_tree_large, "rpp", "tpr")
#lift_log <- performance(pred_log, "rpp", "tpr")
...
xaxis = 0.005*(0:200)
 yaxis = sapply(xaxis, function(prob){useonly = which(probs >= 1-prob); all1s=sum(validation_actual); sum(validation_actual[useonly])/all1s })
#plot(xaxis*100, yaxis*100, type="o")
frame<-cbind(xaxis,yaxis)
frame<-as.data.frame(frame)
LineFrame   <- gvisLineChart(frame, xvar=c("xaxis"), yvar="yaxis", options=list(title='Lift Curve for validation data CART', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFrame,'chart')

```
<center>
```{r lift1,echo=FALSE,results='asis',warning=FALSE,error=FALSE}
...
xaxis = 0.005*(0:200)
 yaxis = sapply(xaxis, function(prob){useonly = which(probs >= 1-prob); all1s=sum(validation_actual); sum(validation_actual[useonly])/all1s })
#plot(xaxis*100, yaxis*100, type="o")
frame1<-cbind(xaxis,yaxis)
frame1<-as.data.frame(frame1)
LineFrame1   <- gvisLineChart(frame1, xvar=c("xaxis"), yvar="yaxis", options=list(title='Lift Curve for validation data CART large', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFrame1,'chart')

```

<center>
```{r lift2,echo=FALSE,results='asis',warning=FALSE,error=FALSE}
####validation data for lift curve logistic regression
#lift_tree <- performance(pred_tree, "rpp", "tpr")
#lift_tree_large <- performance(pred_tree_large, "rpp", "tpr")
#lift_log <- performance(pred_log, "rpp", "tpr")
#lift_tree@x.values[[1]] <- lift_tree@x.values[[1]]*100
...
xaxis = 0.005*(0:200)
 yaxis = sapply(xaxis, function(prob){useonly = which(probs >= 1-prob); all1s=sum(validation_actual); sum(validation_actual[useonly])/all1s })
#plot(xaxis*100, yaxis*100, type="o")
frame2<-cbind(xaxis,yaxis)
frame2<-as.data.frame(frame2)
LineFrame2  <- gvisLineChart(frame2, xvar=c("xaxis"), yvar="yaxis", options=list(title='Lift Curve for validation data CART large', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
...
<div class="col-md-6">
```{r echo=FALSE,results='hide',include=FALSE,warning=FALSE,error=FALSE}
Profit_Matrix = matrix(c(100, -100, -10, 0), ncol=2)
...
framev1<-cbind(xaxis,yaxis_tree)
framev1<-as.data.frame(framev1)
LineFramev1   <- gvisLineChart(framev1, xvar=c("xaxis"), yvar="yaxis_tree", options=list(title='Profit Curve for validation data CART', legend="right", width=600, height=600, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
...
framev2<-cbind(xaxis,yaxis_tree_large)
framev2<-as.data.frame(framev2)
LineFramev2   <- gvisLineChart(framev2, xvar=c("xaxis"), yvar="yaxis_tree_large", options=list(title='Profit Curve for validation data CART large', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
...
framev3<-cbind(xaxis,yaxis_log)
framev3<-as.data.frame(framev3)
LineFramev3   <- gvisLineChart(framev3, xvar=c("xaxis"), yvar="yaxis_log", options=list(title='Profit Curve for validation data logistic regression', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
...
cmp<-table(test_data[,dependent_variable], test_prediction_best,
           dnn=c("Actual", "Predicted"))
confp<-xtable(cmp,caption="Confusion matrix percentages for best Hit Rate classifier: Test Data")
print(confp,type="html",html.table.attributes = "class='table table-striped table-hover table-bordered'",caption.placement="top")
...
###FOR TEST DATA
plot(performance(pred_tree_large1, "tpr", "fpr"), col="red", lty=1, add=FALSE)
grid()
par(new=TRUE)
plot(performance(pred_log1, "tpr", "fpr"), col="blue", lty=1, add=FALSE)
par(new=TRUE)
plot(performance(pred_tree1, "tpr", "fpr"),  lty=1, add=FALSE, main="ROC Curve")
par(new=FALSE)
...
#plot(lift_tree,  lty=1, xlab="Caseload (%)", add=FALSE, main="Lift Curve")
#grid()
#par(new=TRUE)
#plot(lift_tree_large, col="red", lty=1, xlab="Caseload (%)", add=FALSE, main="Lift Curve")
#par(new=TRUE)
#plot(lift_log, col = "blue", lty=1, xlab="Caseload (%)", add=FALSE, main="Lift Curve")
#par(new=FALSE)
...
xaxis = 0.005*(0:200)
 yaxis = sapply(xaxis, function(prob){useonly = which(probs >= 1-prob); all1s=sum(test_actual); sum(test_actual[useonly])/all1s })
#plot(xaxis*100, yaxis*100, type="o")
framel1<-cbind(xaxis,yaxis)
framel1<-as.data.frame(framel1)
LineFramel1   <- gvisLineChart(framel1, xvar=c("xaxis"), yvar="yaxis", options=list(title='Lift Curve for test data CART large', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFramel1,'chart')
...
xaxis = 0.005*(0:200)
 yaxis = sapply(xaxis, function(prob){useonly = which(probs >= 1-prob); all1s=sum(test_actual); sum(test_actual[useonly])/all1s })
#plot(xaxis*100, yaxis*100, type="o")
framel2<-cbind(xaxis,yaxis)
framel2<-as.data.frame(framel2)
LineFramel2   <- gvisLineChart(framel2, xvar=c("xaxis"), yvar="yaxis", options=list(title='Lift Curve for test data CART', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFramel2,'chart')

```
<br>
```{r echo=FALSE,results='asis',warning=FALSE,error=FALSE}
####TEST DATA CART for logistic regression
#lift_tree <- performance(pred_tree, "rpp", "tpr")
#lift_tree_large <- performance(pred_tree_large, "rpp", "tpr")
#lift_log <- performance(pred_log, "rpp", "tpr")
#lift_tree@x.values[[1]] <- lift_tree@x.values[[1]]*100
...
xaxis = 0.005*(0:200)
 yaxis = sapply(xaxis, function(prob){useonly = which(probs >= 1-prob); all1s=sum(test_actual); sum(test_actual[useonly])/all1s })
#plot(xaxis*100, yaxis*100, type="o")
framel3<-cbind(xaxis,yaxis)
framel3<-as.data.frame(framel3)
LineFramel3  <- gvisLineChart(framel3, xvar=c("xaxis"), yvar="yaxis", options=list(title='Lift Curve for test data CART large', legend="right", width=600, height=400, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFramel3,'chart')
...
prob_tree = test_Probability_class1_tree
prob_tree_large = test_Probability_class1_tree_large
prob_log = test_Probability_class1_log
...
frame10<-cbind(xaxis,yaxis_tree)
frame10<-as.data.frame(frame10)
LineFrame10   <- gvisLineChart(frame10, xvar=c("xaxis"), yvar="yaxis_tree", options=list(title='Profit Curve for test data CART', legend="right", width=900, height=600, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFrame10,'chart')
...
frame12<-cbind(xaxis,yaxis_tree_large)
frame12<-as.data.frame(frame12)
LineFrame12   <- gvisLineChart(frame12, xvar=c("xaxis"), yvar="yaxis_tree_large", options=list(title='Profit Curve for test data CART large', legend="right", width=900, height=600, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFrame12,'chart')
...
frame11<-cbind(xaxis,yaxis_log)
frame11<-as.data.frame(frame11)
LineFrame11   <- gvisLineChart(frame11, xvar=c("xaxis"), yvar="yaxis_log", options=list(title='Profit Curve for test data logistic regression', legend="right", width=900, height=600, hAxis="{title:'What here?', titleTextStyle:{color:'black'}}", vAxes="[{title:'What here?'}]",  series="[{color:'green',pointSize:12, targetAxisIndex: 0}]"))
print(LineFrame11,'chart')
...
**Is the performance what we would expect given the performance in the validation data? Should the performance in the test data be close to that in the validation data? More important: should we expect the performance of our classification model to be close to that in our test data when we deploy the model in practice? Why or why not?**
