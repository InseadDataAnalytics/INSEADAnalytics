...
m1<-gvisTable(dfnew,options=list(showRowNumber=TRUE,width=1024, height=600,allowHTML=TRUE,page='disable'))
print(m1,'chart')
```
</div>
...
#show_data = show_data[1:min(max_data_report,nrow(show_data)),]
row<-rownames(show_data)
dfnew2<-cbind(row,show_data)
change<-colnames(dfnew2)
change[1]<-"Variables"
colnames (dfnew2)<-change
m2<-gvisTable(dfnew2,options=list(showRowNumber=TRUE,width=1024, height=600,allowHTML=TRUE,page='disable'))
print(m2,'chart')
```
</div>
...
#show_data = show_data[1:min(max_data_report,nrow(show_data)),]
row<-rownames(show_data)
dfnew3<-cbind(row,show_data)
change<-colnames(dfnew3)
change[1]<-"Variables"
colnames (dfnew3)<-change
m3<-gvisTable(dfnew3,options=list(showRowNumber=TRUE,width=960, height=450,allowHTML=TRUE,page='disable'))
print(m3,'chart')
```
</div>
</div>
...
<style="width=1100px;">
```{r echo=FALSE, message=FALSE, warning=FALSE,prompt=FALSE, results='asis',fig.height=7,fig.width=14}
par(mfrow=c(2,2))
...
dfnew<-cbind(row,show_data)
change<-colnames(dfnew)
change[1]<-"Predictions"
colnames (dfnew)<-change
m1<-gvisTable(dfnew,options=list(showRowNumber=TRUE,width=960, height=140,allowHTML=TRUE,page='disable'))
print(m1,'chart')
...
(red: CART 1; black: CART 2; blue: logistic regression): 
```{r echo=FALSE}
plot(performance(pred_tree_large, "tpr", "fpr"), col="red", lty=1, add=FALSE)
grid()
par(new=TRUE)
...
## Next Class: Class Projects Presentations
